...
import { getGroups, getSeason, loadGroup, teamPlayerStats, playerLabel } from "@/lib/stats";
...
export default async function TeamPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const played = mine.filter((m) => m.status === "played" || m.status === "walkover").slice(-5);
  const upcoming = mine.filter((m) => m.status === "scheduled" || m.status === "postponed").slice(0, 3);
